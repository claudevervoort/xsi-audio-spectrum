...
//
// The Wave is loaded and kept around until the file path is changed.
// The spectrum decomposition is evaluated at the Object level and shared
// across particles. That is to avoid a distinct FFT per particle.
//
...
CStatus RegisterWaveToSpectrum( PluginRegistrar& in_reg )
{
  ...
  // The time is passed using the siICENodeContextSingleton that is it cannot be driven for a single point
  ...
  // The frequency location (normalized from 0-1, o beginning of spectrum, 1 end) is defined at any element level, that is each point for example can represent a different spectrum location
  // Since there are actually a limited amount of bands, linear interpolation is used.
  ...
  // An array of all bands, at the Object scope level
  ...
  // A single frequency power value, based on the IN_frequency node
 ...
 * implementation. We initiate a shared variable here so that the Wave
...
XSIPLUGINCALLBACK CStatus WaveToSpectrum_SubmitEvaluationPhaseInfo( ICENodeContext& in_ctxt )
{
	...
	switch( nPhase )
	{
		case 0:
		{
      // Phase 0 single thread: we set up the wave file and compute the FFT, which is done once for the object.
...
SICALLBACK WaveToSpectrum_Evaluate( ICENodeContext& in_ctxt )
{
  ...
  try {
    if ( 0 == nPhase )
    {
      // in single threaded phase, we compute the FFT to be used then in parallel for each point (phase 1)
    ...
    if ( NULL != spectrumAnalyzer )
    {
      ...
      if ( allBands )
      {
        // the output is the array of all band values (either linear or log frequency intervals)
      ...
      else
      {
        ...
        for(CIndexSet::Iterator it = indexSet.Begin(); it.HasNext(); it.Next())
        {
          // if time was to be an per point basis, then the FFT could no longer be calculated in phase 0
          // it would mean the the analyzer instantiation found in WaveToSpectrum_ComputeFFT would have to
          // be extracted. One way to approach it would be:
          // phase 0: make sure the wave is properly loaded (waveLoader init)
          // phase 1: for each particle, something like:
          //      WaveSpectrumAnalyzer* spectrumAnalyzer = getAnalyzer( in_ctxt );
          //      int block =  userData->waveLoader->toBlockFromTime( timeData[ it ] ); // the time for this particle
          //      bands = spectrumAnalyzer->getFFTBands(block, channel ) ;
          // in that case each particle runs its own FFT, which can be costly.
          float* bands = spectrumAnalyzer->getFFTBands( ) ;
